...
// allowedTaskCols is the whitelist of column names permitted in UpdateTask.
var allowedTaskCols = map[string]bool{
        "title": true, "description": true, "due_date": true,
        "due_time": true, "priority": true, "status": true,
        "sort_order": true, "assignee_id": true, "updated_at": true,
}

// UpdateTask applies partial updates to a task identified by id.
// The updates map keys correspond to column names (snake_case).
// When status is changed to a backlog column, assignee_id is also set to NULL.
func (s *Store) UpdateTask(id string, updates map[string]interface{}) error {
        ...
        // Build SET clause dynamically with column whitelist validation.
        setParts := make([]string, 0, len(updates))
        args := make([]interface{}, 0, len(updates)+1)
        for col, val := range updates {
                if !allowedTaskCols[col] {
...
func scanTask(rows *sql.Rows) (*Task, error) {
        ...
        if parsed, err := time.Parse(time.RFC3339, createdAt); err == nil {
                ...
                fmt.Printf("[store] warn: failed to parse task created_at %q: %v\n", createdAt, err)
        }
        if parsed, err := time.Parse(time.RFC3339, updatedAt); err == nil {
                t.UpdatedAt = parsed
        } else if updatedAt != "" {
                fmt.Printf("[store] warn: failed to parse task updated_at %q: %v\n", updatedAt, err)
        }
        return t, nil
}

// join is a local alias for strings.Join (avoids import cycle in tests).
